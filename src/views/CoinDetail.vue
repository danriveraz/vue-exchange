    ...
    <template v-if="!isLoading">
      <div class="flex flex-col sm:flex-row justify-around items-center">
        ...
        <div class="my-10 sm:mt-0 flex flex-col justify-center text-center">
          ...
          <span class="text-xl"> 
            {{convertResult}} {{ fromUsd ? `USD a ${asset.symbol}` : `${asset.symbol} a USD` }} 
          </span>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    avg() {
      return this.history.reduce((a,b) => a + 
      parseFloat(b.priceUsd), 0) / this.history.length;
    }
  },
    ...
    getWebSite(exchange){
      ...
      return api.getExchange(exchange.exchangeId)
        .then(res => {
          ...
          this.$set(exchange, 'url', res.exchangeUrl) //para solucionar problemas de reactividad en vue.js
        })
        .finally(()=> {
